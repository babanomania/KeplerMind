...
* [x] `app/graph.py`:
  ...
  * [x] Wire all nodes: `intake → research → build_rag → planner → ask_and_score → reflect_and_repair → profile → explain → memorize → schedule → report`.
...
* [x] Add “orbit” icon or ASCII logo:
